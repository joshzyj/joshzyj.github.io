...
- **范例检索**（§14.2 的 Step 0a-exemplars）会从你的策展库退回到按项目扫 Zotero；如果 Zotero 也没有，就退到什么都没有。没有范例喂进去的行文，平庸程度是可测量的 —— 这正是 §20G 存在的机制。 **§14.3 讲的就是怎么把这件事一劳永逸地修好**，包括用你自己已发表的论文去喂这个库。
...
到这里为止的一切，都只是让草稿**正确**。这一小节要让它听起来像**你**，而不是像一个在写社会学的语言模型。这是 §14 里杠杆最大的一件事，也是最多人从来不做的一步。

有两套相互独立的机制在起作用，作用时机不同：

| 机制 | 时机 | 做什么 |
|---|---|---|
| **范例（exemplar）** | 起草**之前** —— `scholar-write` 的 Step 0a-exemplars | 给模型看真实段落，让它模仿其**形状** |
| **微观笔法（tiny topos）** | 起草**之后** —— `/scholar-polish`（§17） | 往成稿里注入作者个人的微观笔法 |

两者之中范例更有力，因为「形状」是在起草时就定下的，事后再改代价很高。

#### 14.3.1 什么是范例

它不是模板，也不是拿来抄的文字。每一份范例都是一段真实发表过的段落，外加一段关于**它好在哪**的标注：
...
**「What it does well」那一块才是承重的部分** —— 抽取契约明确禁止「行文清晰」这类空泛夸奖。而摘录只被拿来学结构：`expansion-quality-check.sh` 会检测与范例的 10-gram 重叠并让整轮失败，所以它不可能渗进你的手稿。

#### 14.3.2 哪些是现成的，哪些得你自己建
...
如果你投的期刊不在这张表上，**你就没有范例，而且没有人会提醒你** —— 起草会静默退回到按项目扫 Zotero，再退到什么都没有。
...
| 模式 | 来源 | 用来做什么 |
|---|---|---|
| **`user-work`** | 你自己的已发表论文 —— Zotero 里打了 `My Publications` 标签的条目 | **你的声音。从这里开始** |
| **`top50`** | `~/.claude/scholar-knowledge/top-50.bib` —— 你最想写成那样的论文 | 你向往的声音 |
...
> **`user-work` 的优先级高于其他一切。** 当同一个「期刊 × 章节」有多个候选时，user-work 的范例会被**排在最前面** —— 技能给出的理由是：*你自己已发表的声音，就是你下一篇论文最贴切的模板*。只要你发表过东西，这就是整本手册里价值最高的一条命令。

先在 Zotero 里把自己的论文打上 `My Publications` 标签（或者用 `--user-tag <tag>` 指向别的标签，也可以设 `SCHOLAR_USER_WORK_TAG`）。

`top50` 的清单要你自己建 —— 它默认不存在，该模式会提示你创建：

```bash
$ ${EDITOR:-nano} ~/.claude/scholar-knowledge/top-50.bib
```

就是一个普通 BibTeX 文件，每条带一个指向 PDF 的 `file = {…}` 字段。五十是上限，不是目标；十篇你真心佩服的，胜过五十篇你只是引用过的。

#### 14.3.4 没有你点头，任何东西都进不了库
...
会逐条走过每个候选 —— frontmatter、摘录、「它好在哪」—— 然后等你按 `[a]ccept` / `[r]eject` / `[s]kip`。拒绝会记下理由，同一篇论文 × 同一章节不会被二次提议。

预算要现实：一本期刊、全部章节、20 篇候选论文，会派出多达约 140 个抽取智能体，大概跑五分钟。通过率预计 **30–50%**。稳态是每个「期刊 × 章节」5–15 份范例。
...
`SOURCE=curated` 说明你的库正在喂给草稿。`SOURCE=fallback` 说明库里对这个组合没有东西，它在按项目扫 Zotero。而 `COUNT=0`，正是这一整节要防的那种无声失败。

这个库是**跨项目、可累积**的：策展一次，之后你为该期刊写的每一篇论文都会对着它起草。它是这条流水线里唯一会**复利**的部分。

**自检：** 跑一次 `/scholar-exemplar-curate user-work`，从你自己最好的那篇论文里通过三个段落，然后起草一节，读出声来。如果还是不像你，那才是 `/scholar-polish`（§17）该上场的地方 —— 但先把形状修对，因为 polish 修不了结构。
