...
├── user1-articles/         ← 你自己已发表的论文 —— 声音、框架、贡献陈述方式
├── user2-articles/         ← 第二作者的作品（合作者，或你自己另一个子领域）
...
> **这里 README 是过时的 —— 以技能为准，不要以文档为准。** 仓库 README 让你建 `example-articles/`。而随包发布的 `SKILL.md` 与 `index.md` 读的是 **`user1-articles/`**。一个叫 `example-articles/` 的目录会被直接忽略。（`user2-articles/` 在 README 里完全没提；它的存在是为了把第二种声音 —— 合作者，或你自己在另一个子领域的作品 —— 单独放开。）
...
1. 把你自己已发表的 PDF 复制进 `user1-articles/`。
...
以后每次加论文就重跑一次。一个已经喂饱的库大致长这样：**32** 篇用户论文、**8** 篇第二作者论文、**87** 篇覆盖 15+ 本期刊的顶刊范文 —— 支撑起一份 1,300 行的知识库和一份 690 行的片段库。
...
**自检：** 把你自己的三篇论文放进 `user1-articles/`，跑一次索引提示词，然后起草一节，读出声来。如果还是不像你，那才是 `/scholar-polish`（§17）该上场的地方 —— 但先把架构修对，因为 polish 修不了结构。
...
这个语料库不是抽象的：它就是 §14.3.1 里的文章库。`assets/article-knowledge-base.md` 里存着这些预算所依据的各期刊实证章节字数，并带有逐篇明细（ASR n=10、AJS n=13、NHB n=6、Science Advances n=4）。把你自己目标期刊的论文加进去，对该刊的校准就会变好。
