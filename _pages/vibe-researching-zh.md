...
4. 把全部 42 个技能与 22 个 agent 装成**个人级** skills，落在 `~/.claude/skills/` 与 `~/.claude/agents/` —— 这样 `/scholar-*` 在**任何**目录、任何 Claude Code 会话里都能用，而不只是在 clone 出来的仓库里。
...
```bash
$ ls ~/.claude/skills/ | grep scholar | wc -l      # 应该是 42
...
每一道关卡都是一个普通的 shell 或 Python 脚本。用它们不需要 Claude 在回路里，而亲手跑一遍是建立对这套系统信任的最快方式：
...
| **事前验尸** | `premortem`、`design review`、`review blueprint` | Step 11 —— 审稿人面板对着现成蓝图开火 |
...
> **溯源要求 —— 这一条是最该刻进脑子里的。** 每份事前验尸备忘都必须带一张**评审溯源表**，列是实打实的：`reviewer_agent | task_invocation_id | dispatched_at | model`。写 `TBD` 或 `—` 这种占位符一律不过关，而且 `design-review-check.sh` 会交叉检查过程日志里是不是零次 `peer-reviewer-*` 派发。技能把「内联扮演的评审」—— 模型用自己的口气写一份“评审”，而不是真的派出子智能体 —— 点名为整条流水线的**头号无声失败**。一次事后复盘把大约 71% 的手稿缺陷追溯到了被跳过或被伪造的评审。§13 的代跑披露就是同一个失败模式，被抓到并声明出来的版本。

我们这次实际运行中，senior 评审标出：*“Y1（接入二值）不该是 headline 结果。Y2（使用强度）才是理论上真正独特的贡献，应该作为摘要句的焦点。”* 这一条评论**在任何分析跑起来之前**就改掉了焦点结果，省掉一整轮修改。

### 9.5 Step 12 —— 章节蓝图

跑得很晚 —— 在结果锁定之后、动笔之前 —— 值得注意的是它是**确定性的**：`generate-section-blueprint.sh` 读取结果锁定、设计蓝图和假设，产出这份契约，**回路里没有 LLM**。写作技能没法跟它讨价还价。
...
- 事前验尸备忘 —— 读每一份的 `TOP-3 FATAL CONCERNS`，并检查溯源表里是不是真的 invocation ID。
...
| **5 —— 事前验尸** | `premortem`、`pre-mortem`、`analysis review` | 不碰数据；针对已起草的脚本派出评审智能体，按 14 个维度的评分表打分 |
...
任何模块加载之前先触发三道关卡：**因果关键词关卡**（先路由到 `/scholar-causal`）、**事前验尸关卡**，以及**仿真关卡**——约 25 个关键词（`abm`、`schelling`、`mesa`、`netlogo`、`opinion dynamics`、`silicon sampling`、`homo silicus` ……）会重定向到 `/scholar-simulate`，因为 ABM 和合成受访者已经搬到那里了。
...
- **范例检索**（§14.2 的 Step 0a-exemplars）会从你的策展库退回到按项目扫 Zotero；如果 Zotero 也没有，就退到什么都没有。没有范例喂进去的行文，平庸程度是可测量的 —— 这正是 §20G 存在的机制。
...
### 14.2 它按什么顺序读什么 —— 以及冲突时谁说了算

这是理解这个技能最重要的一点：`scholar-write` 在写下第一个字之前，要跑一段很长的 Step 0。

| 步骤 | 读取 | 作用 |
|---|---|---|
| **0a-lock** | `results-locked/LATEST.txt` | 验证锁；**验证不过就拒绝起草（exit 1）**；把表格与图钉在锁定快照上 |
| **0a-safety** | `.claude/safety-status.json` | 遇到 `NEEDS_REVIEW` / `HALTED` / `LOCAL_MODE` 就停 |
| **0a-blueprint** | `drafts/section-blueprint.json` | §9.5 里那份逐节写作契约 |
| **0a-revision-directive** | Phase 11.5 / 7b 的指令 | 切换到 expansion 或 patch 模式；超过 24 小时的指令一律忽略 |
| **0a-exemplars** | 已策展的范例库（§20E） | 目标期刊的真实段落，**只学形，不学内容** |
| **0a-outcomes** | `reader_outcomes[]` | 读者读完之后必须能*做*到什么 |
| **0a-lrh** | `drafts/scholar-lrh-*.md` | **对理论章节具有约束力** |
| **0b** | 写作协议与语域指南 | 建立文章知识库与已验证引用池 |

这些互相冲突时，由优先级阶梯裁决：

| 层级 | 内容 | 状态 |
|---|---|---|
| 1 | 字数预算 · 必须检验的假设 · **读者产出** | **强制** |
| 1.5 | 读者产出验证器 | **强制，且由机器检查** |
| 2 | 范例路径、派生动作 | 文字节奏的主要来源 |
| 3 | 结构性动作 | 兜底建议 |
| 4 | 禁止模式 | **硬约束** |

其中两条值得特别强调：

**理论章节被绑定在文献综述上。** 起草理论时，技能会定位 Phase 2 的 `scholar-lrh-*.md` 文件，并把它当作权威。**其中点名的引用至少 70% 必须延续**到理论正文或 `.bib` 里，由 `lit-review-carry-forward-check.sh` 强制执行。丢掉任何一条都要记录理由（`superseded` / `duplicate` / `low-quality` / `out-of-scope`）。技能不得从自己的训练数据里重新推导引用。就是这一条规则，让起草出来的理论章节引用的是你真读过的文献，而不是模型隐约记得的文献。

**范例只用来学形，绝不照抄。** `expansion-quality-check.sh` 会检测与范例的 10-gram 重叠，一旦命中就让这次运行失败。
...
| Demography | 8,000–12,000 | ~150 | —— | 1,500–2,500（文献并入） | —— | —— | —— |
| JMF | 7,000–9,000 | 200 | —— | —— | —— | —— | —— |
| Science Advances | 5,000–8,000 | ~250 | —— | *（无独立理论节；结果在方法之前）* | —— | —— | —— |
| NHB / NCS | 3,000–5,000 | ≤150 | 400–500，不加小标题 | —— | —— | —— | —— |

长文期刊有硬下限：讨论 ≥ 1,200 词，结论 ≥ 500 词。这张表是拿 53 篇以上已发表论文的语料校准出来的 —— 技能自己也记录了：它早先手工整理的那版表格系统性地**低估**了这些区间，现已修正。
...
### 14.6 真实摘要（来自 `drafts/manuscript-final-...md`）
...
每份报告还会在前 30 行里钉上 `manuscript-sha256:` 与 `manuscript-path:`。这堵住了一个真实的 bug：没有 SHA 钉住时，某个关卡可能会重新标出一个后续 polish 已经修掉的 CRITICAL，于是流水线在阶段之间无限回路。这个失败在 2026 年 5 月的一个真实项目上被观察到过。
...
技能会追猎二十四种有名有姓的套路（tells）。就算你不用这个工具，认识它们也会让你写得更好：
...
- **T20 是本语料库里观察到的最大实际缺陷** —— 比 T1–T19 加起来还大。2026 年 5 月的一份草稿，单篇手稿里就带出了 13 个 `L-A#` 标记、8 处 `spec_id` 伪引用，以及 12 个无处安放的 `M#`/`R#` 引用。

> **一个值得知道的文档 bug。** 技能自带的质量清单至今仍写着「已扫描全部 18 类套路（T1–T18）」，但目录实际排到 T24。照清单字面执行会少扫六类。请显式要求它扫 T19–T24。
...
- **主题轮换。** 每个审稿人至多拿到一个引导主题，主题不重复使用，资深审稿人永远不被引导。引导主题来自四个独立来源（Phase 7b 警告、设计事前验尸的红队标记、上一轮未解决项、文献综述缺口）。没有这套机制，几个「独立」审稿人会因为被指向同一件事而收敛到同一个意见。
...
## 20. `scholar-presentation` —— 演讲与印刷海报
...
## 20A. `scholar-image` —— 只做装饰性图像，以及一条硬性拒绝
...
## 20G. `scholar-exemplar-curate` —— 教写作者什么叫好
...
当 Phase 7b 的验证发现正文误述了某张表，修复的地方在 Phase 7，不在 7b。关卡会发出一条**回路指令（back-route directive）**：
...
回路从 Phase 5C、7b、8、11.5 的 RED 分支发出。多条同时触发时，**最上游的目标胜出**。两道刹车防止无限循环：数值上限 **9**（在自主循环下降为 **2**），以及**同签名上限 3**——同一组发现回路三次之后，流水线升级上报，而不是试第四次。计数器只在真正向前推进时才归零。
...
- 某个事前验尸的 RED 点名了一个不在 lock manifest 里的产物。
- 某位审稿人的文字判定与它自己的结构化元数据互相矛盾。

## 21A. `scholar-resume` —— 我在哪，下一步跑什么
...
| `back-route-escalate:<phase>:<reason>` | 回路计数器已到上限 | **停。** 原样呈报。不要再路由 |
...
- **验证戳。** `auto-research-verify.sh` 为每个通过的阶段铸一枚戳。没有戳就标记阶段完成，会以 `VERIFY_STAMP_REQUIRED` 失败；戳与产物不再匹配，则以 `VERIFY_STAMP_STALE` 失败。回路会**删除**所有被作废的下游阶段的戳，所以你不可能不小心留着一个过期的通过。
- **显式运行模式。** 模式初始为 `unset`，在你选定之前 `next` 一律返回 `MODE_SELECTION`。在 `human_in_loop` 下，每一次阶段转换都会创建一个待决决策，在你批准之前阻塞完成。**自主模式绝不会从沉默中推断出来。**

回路上限是**每个 finding ID 3 次**；第三次以退出码 2 结束，要求人类去检查底层问题，并手工清空重试计数。
...
| auto-research 回路升级，或 `APPROVAL_REQUIRED=1`，或运行模式未设定 | **blocked** |
